...
async def format_nation_info(
    ...
    match shard:
        ...
        case "demonym2plural":
            await interaction.response.send_message(
                embed=discord.Embed(title=f"A group of citizens from {nation} are called {text}")
            )
        case "dispatches":
            await interaction.response.send_message(
        ...
        case "scvote":
            if text:
                ...
                await interaction.response.send_message(
                    embed=discord.Embed(title=f"{nation} is not a member of the Security Council.")
                )
        case "sectors":
            if text is None:
        ...
        case "tgcanrecruit":
            if text == "1":
                ...
                await interaction.response.send_message(
                    embed=discord.Embed(title=f"{nation} can not receive recruitment telegrams.")
                )
        case "tgcancampaign":
            if text == "1":
                await interaction.response.send_message(
