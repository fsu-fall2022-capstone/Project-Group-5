...
class FormatWAInfo(Formatter):
    @classmethod
    async def format(cls, council, shard: str, data: str, interaction: discord.Interaction) -> None:
        ...
        match shard:
            case "numnations":
                await interaction.response.send_message(
                    embed=discord.Embed(
                        title=f"The {council} has a whooping {text} member nations!"
                    )
                )
            case "numdelegates":
                await interaction.response.send_message(
                    embed=discord.Embed(
                        title=f"There are currently {text} people serving as delegates to the {council}!"
                    )
                )
            case "delegates":
